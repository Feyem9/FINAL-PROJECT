...
export const Chats = ({ role, senderId, receiverId }) => {
  ...
  useEffect(() => {
    ...
    // addDebugLog(`Utilisateur connecté: ${userId} (${userRole})`);
  ...
  useEffect(() => {
    ...
    socket.on("receiveMessage", (msg) => {
      // addDebugLog(
        // `📩 Message reçu de ${msg.senderId}: ${msg.message.substring(0, 50)}...`
      // );
      const formattedMsg = {
        ...msg,
        id: msg._id || uuidv4(),
    ...
    socket.on("userStatus", (status) => {
      // addDebugLog(
      //   `👤 Statut ${status.userId}: ${status.isOnline ? "Online" : "Offline"}`
      // );
    ...
    console.log('userRole', userRole);
      ...
      axios
        .get(`${databaseUri}/${userRole}s/${receiverId}`)
        .then((res) => {
        //   // addDebugLog(
        //   //   `✅ Utilisateur récupéré: ${res.data.name || res.data._id}`
        //   // );
  ...
  const sendMessage = () => {
    ...
    if (!senderId || !receiverId) {
      // addDebugLog(
      //   `❌ IDs manquants: senderId=${senderId}, receiverId=${receiverId}`
      // );
    ...
    setMessages((prev) => {
      // addDebugLog(
      //   `📝 Ajout message local: ${prev.length} -> ${prev.length + 1} messages`
      // );
  ...
  const uploadFile = async (file) => {
    ...
    try {
      const res = await axios.post(`${databaseUri}/chat/upload`, formData, {
        ...
        {/* {(
          <div className="bg-black text-green-400 p-4 rounded-lg mb-4 text-xs font-mono max-h-48 overflow-y-auto">
            ...
            <div className="mt-2 pt-2 border-t border-gray-600 text-xs">
              <div>
                Socket: {socket ? "✅" : "❌"} | Connecté:{" "}
                {isConnected ? "✅" : "❌"}
              </div>
              <div>
                Messages: {messages.length} | Admin: {admin ? "✅" : "❌"}
              </div>
            </div>
          </div>
        )} */}
              ...
              <div className="p-3 bg-blue-100 rounded-full">
                ...
                >
                  <path
                    ...
                    strokeLinejoin="round"
                    strokeWidth={2}
                    d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a1 1 0 11-2 0 1 1 0 012 0zM7 10a1 1 0 11-2 0 1 1 0 012 0z"
                  />
                </svg>
              </div>
            </div>
            ...
            <div className="flex flex-col h-[80vh] w-full max-w-full mx-auto border border-gray-200 rounded-2xl shadow-xl bg-white overflow-hidden">
              ...
              <div className="relative flex items-center p-4 border-b border-gray-200">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-2">
                    <div
                      className={`w-2 h-2 rounded-full ${isConnected && onlineStatus === "Online" ? "bg-green-500" : "bg-gray-400"}`}
                    ></div>
                    <span
                      className={`text-sm ${onlineStatus === "Online" && isConnected ? "text-green-500" : "text-gray-500"}`}
                    >
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-gray-50">
                ...
                {messages.map((m) => (
                  ...
                  >
                      ...
                      {m.isFile && m.fileUrl ? (
                        <div className="flex flex-col gap-2">
                          <div className="flex items-center gap-2">
                            ...
                            {m.metadata?.size && (
                              <span
                                className={`text-xs ${m.senderId === senderId ? "text-gray-200" : "text-gray-500"}`}
                              >
                    ...
                    <span
                      className={`mt-1 text-xs text-gray-400 ${m.senderId === senderId ? "mr-2" : "ml-2"}`}
                    >
                      {m.createdAt
              ...
              <div className="flex items-center p-4 border-t border-gray-200 bg-white gap-2">
                ...
                <button
                  ...
                  title={
                    uploading ? "Upload en cours..." : "Partager un fichier"
                  }
                >
                  {uploading ? "⏳" : "📎"}
                </button>
                ...
                <button
                  ...
                  title={
                    !isConnected ? "Connexion requise" : "Envoyer le message"
                  }
                >
                  {!isConnected ? "⏸️" : "➤"}
                </button>
